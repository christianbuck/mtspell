...
namespace stringdist 
{
  ...
  StringDiff::
  Segment::
  elabel[] = { "same", "cap", "flip", 
	       "permutation", "insertion", "deletion", 
	       "mismatch", "noinit" };
...
#define DEBUGME 0
  ...
  Segment(size_t const as, size_t const ae, 
	  ...
	  UnicodeString const& a, UnicodeString const& b)
  {
    dist = 0;
    start_a = as; end_a = ae; 
    start_b = bs; end_b = be;
    ...
    if      (match == insertion)   dist = be-bs;
    else if (match == deletion)    dist = ae-as;
